...
const schema = yup.object().shape({
  ...
  shortDescription: yup
    ...
    .min(2, "Content must be at least 100 characters"),
  thumbnail: yup.string().required("Thumbnail is required"),
  category: yup.string().required("Category is required"),
  tags: yup
    .array()
    .min(2, "Please select at least more than one tag")
      ...
      <div className="max-w-5xl mx-auto p-8 my-5">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-5">
            ...
            <div
              className="shadow appearance-none border dark:border-borderDark rounded w-full p-3 text-gray-700 dark:text-white leading-tight focus:outline-none focus:shadow-outline"
              id="tags"
            >
              {tags.map((tag, index) => (
                <span
                  key={index}
                  className="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-1 rounded dark:text-blue-400 border border-blue-400"
                >
                  {tag}
                  <button
                    type="button"
                    className="inline-flex items-center p-1 ms-2 text-sm bg-transparent rounded-sm hover:bg-blue-200 dark:hover:text-blue-300"
                    aria-label="Remove"
              ...
              <div className="md:col-span-4">
                <select
                  className="shadow appearance-none border dark:border-borderDark rounded w-full p-3 text-gray-700 dark:text-white leading-tight focus:outline-none focus:shadow-outline mt-2"
